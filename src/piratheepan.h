// ─────────────────────────────────────────────────────────────
//  PIRATHEEPAN — Raster Graphics, Pixel Processing, Z-Buffer & Entertainment
// ─────────────────────────────────────────────────────────────
//  Lecture Concepts:
...
//   Bloom ping-pong FBOs — bloomA <-> bloomB swap 6 times for Gaussian blur
//   Pixel font HUD       — 5x7 bitmap font — each character drawn as pixel rectangles
...
//   Real-time 60 FPS     — game loop runs render every frame — entertainment application
//   Entertainment        — video game = exact entertainment category (lecture slide 24)
// ─────────────────────────────────────────────────────────────
#pragma once

// ── HUD vertex shader — normalised [0,1] -> NDC mapping ──────
...
// ── HUD fragment shader — flat colour output ─────────────────
...
// ─────────────────────────────────────────────────────────────
//  Pixel Font  5x7 bitmap, supports 0-9 and A-Z
...
// Draw a 2D rectangle on screen in normalised [0,1] coords
...
// Draw a single character using the 5x7 pixel font bitmap
...
static void drawSpeedometer(float spd, float maxSpd)
{
    ...
    float zfrac = (camZoom - 0.40f) / (2.20f - 0.40f);
