// ─────────────────────────────────────────────────────────────
//  PIPUNI — Rendering & Graphics Pipeline
// ─────────────────────────────────────────────────────────────
//  Lecture Concepts:
...
// ── Sky gradient vertex + fragment shaders ───────────────────
...
// ── Screen-space quad vertex shader (post-process passes) ────
...
// ─────────────────────────────────────────────────────────────
//  GL helpers: shader compilation & program linking
// ─────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────
//  Render — full pipeline every frame
//  Scene FBO -> sky -> geometry -> bloom -> composite -> HUD
// ─────────────────────────────────────────────────────────────
static void render()
{
    ...
    // Sky gradient colours (day -> night with distance)
    ...
    if(gState==GS_PLAYING||gState==GS_DEAD||gState==GS_WIN){
        ...
        // Coins (glowing discs = small yellow cuboid, spinning each frame)
        for(auto& c:coins){
            if(!c.alive) continue;
    ...
    // ── 6x Gaussian blur ping-pong (bloomA <-> bloomB) ─────────
    bool horiz=true;
    for(int i=0;i<6;i++){
    ...
    // ── Composite: scene + bloom + Reinhard tonemapping ────────
